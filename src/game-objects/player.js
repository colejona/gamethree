...
import { TEAM_RED, TEAM_GREEN, PLAYER_RED, PLAYER_GREEN, PLAYER_SPEED, PLAYER_SPEED_WITH_FLAG } from '../constants'
...
export class Player extends GameObject {
  constructor(scene, x, y, id, team, config={}) {
    super(scene, x, y, team, SPRITE_MAPPING[team]);
    this.facing = UP;
    // TODO: handle error case for sprite mapping
    this.setId(id);
    this.setupAnimationsForSpriteSheet(scene, SPRITE_MAPPING[team]);
    this.sprite.anims.play(this.getAnimationKey(SPRITE_MAPPING[team], PLAYER_IDLE, this.facing));
  ...
  setupAnimationsForSpriteSheet(scene, playerSprite) {
    // Animations are already registered:
  ...
  getSpeed() {
    return this.hasFlag() ? PLAYER_SPEED_WITH_FLAG : PLAYER_SPEED;
  }
  ...
  updateDirection(facing, isStop=false) {
    ...
    if (this.sprite.body.velocity.x || this.sprite.body.velocity.y) {
      this.playAnimIfChanged(this.getAnimationKey(SPRITE_MAPPING[this.team], PLAYER_MOVE, this.facing));
    } else {
      this.playAnimIfChanged(this.getAnimationKey(SPRITE_MAPPING[this.team], PLAYER_IDLE, this.facing));
    }
  }
  ...
  pickUpFlag(flag) {
    ...
    this.carriedFlag.pickUp(flag);
  }

  dropFlag() {
    if (this.carriedFlag) {
      this.removeChild(this.carriedFlag);
      this.carriedFlag.drop();
      this.carriedFlag.resetPosition();
      this.carriedFlag = null;
    }
  }
